...
public class RealFishTracking_Parallax : MonoBehaviour {
    ...
    void Update() {
        if(Time.time > BaselineTime && Time.time < ExpTime){
            ...
            transform.position = new Vector3(-9f, 6.5f,-NowZposition*1.75f+7.7f);
             //transform.position = new Vector3(-9f, 6.5f,-(-NowZposition*1.75f+7.7f)); //Negative parallax
