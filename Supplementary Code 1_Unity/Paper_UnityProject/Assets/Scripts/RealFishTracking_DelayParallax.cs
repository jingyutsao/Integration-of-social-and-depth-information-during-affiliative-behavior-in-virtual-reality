...
public class RealFishTracking_DelayParallax : MonoBehaviour {

    // Set before Exp
    private string Date = "20241205_T4_DP_";
    private float ExpTime = 1200.0f;
    private float BaselineTime = 300.0f;
    int DelayFrames = 630; // Adjustable delay variable
    ...
    private float DelayZposition;

    List<float> pastZpositions = new List<float>();

    private Camera camera;

    // Define the range of the camera's z-position.
    public const float minZ = -7.675f;
    public const float maxZ = 7.675f;
    ...
    void Update() {
        if(Time.time > BaselineTime && Time.time < ExpTime){
            ...
            // Store the current NowZposition and manage the delay
            if (pastZpositions.Count >= DelayFrames)
            {
                DelayZposition = pastZpositions[0];      // Get the Z position from 'delayFrames' ago
                pastZpositions.RemoveAt(0);              // Remove the oldest value to keep the list size at 'delayFrames'
            }
            else
            {
                DelayZposition = NowZposition;           // Default to NowZposition if we don't have enough values yet
            }

            // Add the current NowZposition to the list
            pastZpositions.Add(NowZposition);

            transform.position = new Vector3(-9f, 6.5f,-DelayZposition*1.75f+7.7f);
            // transform.position = new Vector3((-NowXposition*1.75f), 6.5f,-NowZposition*1.75f+7.7f);
