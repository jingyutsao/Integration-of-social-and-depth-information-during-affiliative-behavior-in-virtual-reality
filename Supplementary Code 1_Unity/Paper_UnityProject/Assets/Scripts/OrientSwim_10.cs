...
public class OrientSwim_10 : MonoBehaviour
{   
    ...
    // Set before Exp
    private string Date = "20240806_T8_para";
    ...
    void Awake(){
        ...
        SchoolingTarget = GameObject.Find("Sphere (3)").GetComponent<Transform>();
    ...
    IEnumerator Run(){
        ...
        transform.position = new Vector3(17.85f, 6.5f, 17.0f);
    ...
    while(isProgrammmingRun == true){
        ...
        if(Swimstep == 1){
            ...
            while(Time.time <= SwipingEnd){
                xMax = xMaxWall - HeadToCentroid; xMin = xMinWall + HeadToCentroid + 0.5f;// xMin is larger than xMin in spontaneous mode.
                YMax = YMaxWall; YMin = YMinWall;
                ZMax = ZMaxWall - HeadToCentroid; ZMin = ZMinWall + HeadToCentroid;

                // Angle between two objects
                relativePos = new Vector3 ((transform.position.x - Sphere.position.x), 0.0f, (transform.position.z - Sphere.position.z)).normalized;
                targetHeading = Quaternion.LookRotation(relativePos, Vector3.up);
                DegFuture = targetHeading.eulerAngles.y; 

                /// Swim to real fish
                if(transform.position.x > 4.0f){

                    DegPast = transform.rotation.eulerAngles.y;
                    HeadAngle = DegFuture - DegPast;
                    ReadMotif(); 
                    Array.Resize<string>(ref HeadAngleSave, HeadAngleSave.Length + 1); HeadAngleSave[HeadAngleSave.Length-1] = HeadAngle.ToString();

                    // Rotation
                    targetHeading = transform.rotation;
                    targetHeading *= Quaternion.Euler(Vector3.up * HeadAngle);
                    ...
                    Array.Resize<string>(ref SwimScript, SwimScript.Length + 1); SwimScript[SwimScript.Length-1] = (Swimstep-1).ToString();

                    // Avoid getting stuck near the wall
                    CollisionAvoid();
                    if(Predictdistance < GlideDistancePlus){
                        GlideTime = Predictdistance/ (GlideForce*0.02f);} 

                    CurrentTime = Time.time;
                    //Move forward
                    while(Time.time < CurrentTime + GlideTime){
                        VirtualFish.AddRelativeForce(Vector3.back * GlideForce);
                        transform.position = new Vector3(
                        Mathf.Clamp(transform.position.x, xMin, xMax), 
                        Mathf.Clamp(transform.position.y, YMin, YMax), 
                        Mathf.Clamp(transform.position.z, ZMin, ZMax)
                    ...
                    print("The time for swiping start " + Time.time);
                }

                /// Swipe behavior start
                // Swiping
                else{
                   
                    DegPast = transform.rotation.eulerAngles.y;

                    if(DegFuture <= 90.0f && DegFuture >= 0.0f ){
                        DegFuture = UnityEngine.Random.Range(0.0f,90.0f);
                        //SwipCnt += 1;
                        if(SwipCnt < 0){SwipCnt=0;}
                        if(SwipCnt > 3){
                            DegFuture = UnityEngine.Random.Range(120.0f,180.0f);
                            SwipCnt=0;
                            }
                        }

                    if(DegFuture <= 180.0f && DegFuture > 90.0f ){
                        DegFuture = UnityEngine.Random.Range(90.0f,180.0f);
                        //SwipCnt -= 1;
                        if(SwipCnt > 0){SwipCnt=0;}
                        if(SwipCnt < -3){
                            DegFuture = UnityEngine.Random.Range(10.0f,45.0f);
                            SwipCnt=0;
                            }
                        }
                    ...
                    Array.Resize<string>(ref SwimScript, SwimScript.Length + 1); SwimScript[SwimScript.Length-1] = Swimstep.ToString();     
                    AdjustDistance = Mathf.Abs(Sphere.position.x - transform.position.x);
                    GlideForce = AdjustDistance/0.04f; 

                    CurrentTime = Time.time;
                    
                    //Move forward
                    while(Time.time < CurrentTime + 0.01f){
                        if((DegFuture > 30.0f && DegFuture < 70.0f) || (DegFuture > 110.0f && DegFuture < 150.0f)){
                            VirtualFish.AddRelativeForce(Vector3.back * GlideForce);
                        }
                        if(DegFuture <= 30.0f || DegFuture >= 150.0f){
                            VirtualFish.AddRelativeForce(Vector3.back * GlideForce * 2.0f);
                        }
                        transform.position = new Vector3(
                        Mathf.Clamp(transform.position.x, xMin, xMax), 
                        Mathf.Clamp(transform.position.y, YMin, YMax), 
            ...
            print("The time after swip " + Time.time);
            OnApplicationQuit();
        }
    ...
    void SaveVirtualFishData(){
        string csvFilePath = Date + "Data_10.csv"; // Specify the path and name of the CSV file
    ...
    private void GetTargetSpeed(){
        ...
        lastTime = Time.time;
        // if(TargetSpeed <= 1f){motifSpeed = 1f;}
        // else{motifSpeed = TargetSpeed;}
        motifSpeed = 1f;
